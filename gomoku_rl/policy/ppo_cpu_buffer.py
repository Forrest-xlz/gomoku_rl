...
class PPO(Policy):
    ...
    def __init__(
        ...
        # -------------------- CPU rollout-buffer learning --------------------
        # When a runner collects a very large mixed-teacher batch, keeping the
        # complete rollout TensorDict on CUDA can OOM before PPO even starts.
        # With this enabled, PPO keeps the full rollout/flattened transition
        # buffer on CPU, computes GAE on CPU, and moves only each minibatch to
        # ``self.device`` inside the optimization loop. The runner can also set
        # these attributes dynamically before calling learn().
        self.keep_rollout_buffer_on_cpu: bool = bool(
            cfg.get("keep_rollout_buffer_on_cpu", cfg.get("cpu_rollout_buffer", False))
        )
        self.gae_on_cpu: bool = bool(cfg.get("gae_on_cpu", self.keep_rollout_buffer_on_cpu))
        # --------------------------------------------------------------------
    ...
    def learn(self, data: TensorDict):
        ...
        keep_cpu = bool(getattr(self, "keep_rollout_buffer_on_cpu", False))
        gae_on_cpu = bool(getattr(self, "gae_on_cpu", keep_cpu))
        if keep_cpu:
            # Keep the complete rollout buffer and the flattened transition buffer
            # on CPU. Minibatches are moved to CUDA one at a time below.
            data = data.to("cpu")
            gae_device = torch.device("cpu") if gae_on_cpu else torch.device(self.device)
        else:
            gae_device = torch.device(self.device)

        value = data["state_value"].to(gae_device)
        next_value = data["next", "state_value"].to(gae_device)
        done = data["next", "done"].unsqueeze(-1).to(gae_device)
        ...
        if terminated_raw is None:
            ...
            terminated = terminated_raw.unsqueeze(-1).to(gae_device)
        reward = data["next", "reward"].to(gae_device)
        ...
        with torch.no_grad():
            ...
            if keep_cpu:
                adv = adv.cpu()
                value_target = value_target.cpu()
            data.set("advantage", adv)
            data.set("value_target", value_target)

        # Drop temporary full-batch GAE tensors before minibatch optimization.
        del value, next_value, done, terminated, reward, adv, value_target
        if torch.cuda.is_available() and keep_cpu:
            torch.cuda.empty_cache()

        invalid = data.get("invalid", None)
        if invalid is not None:
            if keep_cpu:
                invalid = invalid.to("cpu")
            data = data[~invalid]
        data = data.reshape(-1)
        ...
        info = {
            ...
            "ppo/keep_rollout_buffer_on_cpu": float(bool(getattr(self, "keep_rollout_buffer_on_cpu", False))),
            "ppo/gae_on_cpu": float(bool(getattr(self, "gae_on_cpu", False))),
        }
